...
namespace CameraControl
{
    ...
    public class Do3ThinkImageAcquirer : IImageAcquirer
    {
        ...
        public static CameraHandle m_iCameraHandle = 0;
        
        private string[] m_sCameraRealNameList;
        string firstCamera = ConfigurationManager.AppSettings["cameraID"];
        string XResolution = ConfigurationManager.AppSettings["xResolution"];
        bool bInitialized = false;
        static object obj = new object();
        dvpCamera.DelegateProc callback;
        static int cameraFrames;
        static Bitmap lastImage;
        delegate void ImageAcquireFinished(Bitmap image);
        event ImageAcquireFinished onFinished;
        static ManualResetEvent snapDone = new ManualResetEvent(false);
        ...
        public static int SnapThreadCallback(int m_iCam, IntPtr pbyBuffer, ref tDSFrameInfo sFrInfo)
        {
            ...
            Debug.WriteLine(m_iCam.ToString());
            //显示RGB24数据到画面
            dvpCamera.CameraDisplayRGB24(m_iCam, pBmp24, ref sFrInfo);
            if (m_iCameraHandle != m_iCam)
                return 0;

            if (cameraFrames > 3) //store the third frame
            {
                try
                {
                    byte[] bData = new byte[sFrInfo.uiWidth * sFrInfo.uiHeight * 3];
                    bData = dvpCamera.IntPtrToByteArray(pBmp24, (int)(sFrInfo.uiWidth * sFrInfo.uiHeight * 3));

                    lastImage = new Bitmap((int)sFrInfo.uiWidth, (int)sFrInfo.uiHeight, PixelFormat.Format24bppRgb);
                    var bitmap_data = lastImage.LockBits(new Rectangle(0, 0, lastImage.Width, lastImage.Height), ImageLockMode.WriteOnly, lastImage.PixelFormat);
                    int lineLength = (int)sFrInfo.uiWidth * 3;
                    Marshal.Copy(bData, 0, bitmap_data.Scan0, bData.Length);
                    lastImage.UnlockBits(bitmap_data);
                }
                catch (Exception ex)
                {
                    log.Info(ex.Message);
        ...
        private void Init()
        {
            ...
            string sName = m_sCameraRealNameList[0];
            callback = SnapThreadCallback;
            emDSCameraStatus status = dvpCamera.CameraInit(callback, sName, IntPtr.Zero, ref m_iCameraHandle);
            if (status != emDSCameraStatus.STATUS_OK)
                throw new Exception(string.Format("无法初始化相机，原因是: {0}", status.ToString()));
            dvpCamera.CameraSetOnceWB(m_iCameraHandle);

            //int resIndex = GetIndex(m_iCameraHandles[i]);
            dvpCamera.CameraSetImageSizeSel(m_iCameraHandle, 0, true);
            var result = dvpCamera.CameraPlay(m_iCameraHandle);
            bInitialized = true;
        }

        private int GetCameraCnt()
        {
            return 1;
        }

        private void CheckCameraNames(List<string> realCameraNames)
        {
            CheckAllowed();

            bool expectedCamerasFound = realCameraNames.Exists(x => x.Contains(firstCamera));
        ...
        private void CheckAllowed()
        {
            ...
            string[] allowedCameraID = { "UE500900519", "UE500900506", "UE500900510", "UE500900516", "UE500900526" };
            bool isAllowed = allowedCameraID.Contains(firstCamera);
            if (!isAllowed)
                throw new Exception(string.Format("摄像头：{0}未注册！", firstCamera));
        }
        ...
        public void Stop()
        {
            ...
            try
            {
                dvpCamera.CameraStop(m_iCameraHandle);
                dvpCamera.CameraUnInit(m_iCameraHandle);
            }
            catch (Exception ex)
            {
                ;
        ...
        public void Start()
        {
            string errMsg = "";
#if DEBUG
            StartImpl( GlobalVars.CameraSettings);
#else
            try
            {
                StartImpl(cameraID, cameraSetting);
            }
            catch(Exception ex)
            {
                errMsg = ex.Message;
            ...
            if (onFinished != null)
                onFinished( AcquiredImage);
        }



        private void StartImpl( CameraSettings cameraSetting)
        {
            Init();
            if (cameraSetting.IsAE)
            {
                dvpCamera.CameraSetAeState(m_iCameraHandle, true);
            }
            else
            {
                ...
                dvpCamera.CameraGetExposureTime(m_iCameraHandle, ref curExposeTime, ref maxExposeTime, ref minExposeTime);
                ...
                dvpCamera.CameraSetAeState(m_iCameraHandle, false);
                dvpCamera.CameraSetAnalogGain(m_iCameraHandle, (float)cameraSetting.Gain);
                dvpCamera.CameraSetExposureTime(m_iCameraHandle, exposeTime);
            }

            cameraFrames = 0;
            snapDone.Reset();
            snapDone.WaitOne();
            AcquiredImage = (Bitmap)ImageProcessor.Resize(lastImage, new Size(1280, 960));
        }
